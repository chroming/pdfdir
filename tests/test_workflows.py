...
def test_linux_workflows_use_shared_qt_runtime_setup():
    ...
    assert f"bash {INSTALL_SCRIPT}" in release_workflow
...
def test_frozen_smoke_uses_absolute_resources_with_external_spec_directory():
    ...
    assert '-i "$GITHUB_WORKSPACE/pdf.ico"' in test_workflow
    assert '--add-data "$GITHUB_WORKSPACE/pdf.ico:src"' in test_workflow
    assert (

...
def test_message_box_uses_real_pyside6_enums_and_exec(
    window, qapp, monkeypatch, level, title, icon
):
    shown_boxes = []
    window_titles = []
    ...
    window.to_english()
    window.alert_msg("Message", level=level)

    assert window_titles == [title]
    assert shown_boxes == [(icon, "Message")]


def test_message_box_ok_action_receives_clicked_button(window, qapp, monkeypatch):
    shown_boxes = []
    clicked = []
    ...
    window.alert_msg("Confirm", ok_action=lambda button: clicked.append(button))
    box = shown_boxes[0]
...
def test_tree_uses_a_stable_qt_subclass(window):
    assert type(window.dir_tree_widget) is BookmarkTreeWidget
    for source_path in GUI_SOURCE_FILES:
        tree = ast.parse(source_path.read_text(encoding="utf-8"))
        assert not any(
